...
# Initialize OpenVINO Core to detect available devices
            ...
            # Set up provider options properly - this was causing the error
            providers = []
            provider_options = []
            
            if has_intel_gpu:
                # Set OpenVINO as primary provider for Intel GPUs
                os.environ['OPENVINO_DEVICE'] = 'GPU'
                providers.append('OpenVINOExecutionProvider')
                provider_options.append({})  # Empty dict for default options
            ...
            if not has_intel_gpu:
                logger.warning("No GPU detected. Running in CPU mode only")
            
            # Initialize face analysis with proper provider configuration
            self.app = FaceAnalysis(
                name="buffalo_l",
                providers=providers,
                provider_options=provider_options,  # This must be a sequence matching providers
